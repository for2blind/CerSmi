...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/llama-7b/LATENCY_OAP/10.0-12/llama-7b-submod-8.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/llama-7b/LATENCY_OAP/10.0-12/llama-7b-inputs-8.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://172.169.8.253:31112/function/llama-7b-submod-9-latency-12.dasheng#"+str(uid), timeout=timeout) as response:
