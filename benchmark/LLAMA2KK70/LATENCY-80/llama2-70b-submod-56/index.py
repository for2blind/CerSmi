...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/llama2-70b/LATENCY_OAP/10.0-80/llama2-70b-submod-56.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/llama2-70b/LATENCY_OAP/10.0-80/llama2-70b-inputs-56.pt", map_location=infer_device)
    output = model(**x)
...
async def call_next_function():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://172.169.8.253:31112/function/llama2-70b-submod-57-latency-80.dasheng#"+str(uid), timeout=timeout) as response:
