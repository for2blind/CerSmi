...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/wholegraphs/llama2-70b//llama2-70b-wholemod-3.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/wholegraphs/llama2-70b/llama2-70b-wholemod-inputs-3.pt", map_location=infer_device)
    output = model(**x)
...
async def call_next_function():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://172.169.8.253:31112/function/llama2-70b-wholemod-4-llama2-70b-whole.dasheng#"+str(uid), timeout=timeout) as response:
