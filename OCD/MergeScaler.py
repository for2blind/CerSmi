...
SCALE_UP_COOLDOWN = 12 * 60  # 10 minutes
...
# global model_nums
# model_nums = 4
# model_nums = os.environ.get('model_nums')
# if model_nums is None:
#     raise ValueError("model_nums must be set")

# model_need_to_patch = [x for x in range(0, 64, model_nums)]


# todo: if deployment is not in model_need_to_patch, then scale to 0
# for example, [1, 2, 4, 8, 16, 32, 64]
def scale_down(expect_qps: float, replicas: list[int], throughtput: list[float], tolerance: float):
    qps = 0
    for i in range(len(replicas)):
        qps += replicas[i] * throughtput[i]
    new_replicas = replicas.copy()
    while qps > expect_qps * tolerance:
        for i in reversed(range(1, len(new_replicas))):
            while new_replicas[i] > 0 and qps - throughtput[i] + throughtput[i-1] >= expect_qps * tolerance:
                new_replicas[i] -= 1
                new_replicas[i-1] += 1
                qps = qps - throughtput[i] + throughtput[i-1]
    while new_replicas[0] > 0 and qps - throughtput[0] > expect_qps * tolerance:
        new_replicas[0] -= 1
        qps -= throughtput[0]
    return new_replicas       
...
def calculate_replicas(rps, b=2):
    normalized_rps = min(MIN_REPLICAS, rps / (8 * 12))
    replicas = easeOutBack(normalized_rps, b=b)
...
async def sdag_scale(namespace):
    ...
    for deployment in deployments.items:
        ...
        desired_replicas = calculate_replicas(rps)

        if rps == 0 and current_scale.spec.replicas == desired_replicas:
