...
async def scale_up_one(deployment_name, namespace, num_replicas):
    try:
        current_deployment = kube_client.read_namespaced_deployment(
            deployment_name, namespace
        )
        current_replicas = int(current_deployment.status.available_replicas)
    ...
    try:
        cmd = [
            ...
            deployment_name,
            "--replicas",
            str(int(current_replicas)+1),
            "--namespace",
            namespace,
        ]
        subprocess.check_output(cmd)
        print(
            f"Successfully scaled deployment {deployment_name} to {int(current_replicas)+1} replicas"
        )
...
async def Infless_scale(namespace):
    ...
    for deployment in deployments.items:
        ...
        # await scale_up_one(deployment_name, namespace, desired_replicas)
